const requestStorageAccess = (shop) => {
  return `(function() {
      function redirect() {
        var targetInfo = {
          myshopifyUrl: "https://${encodeURIComponent(shop)}",
          hasStorageAccessUrl: "/auth/inline?shop=${encodeURIComponent(shop)}",
          doesNotHaveStorageAccessUrl: "/auth/enable_cookies?shop=${encodeURIComponent(shop)}",
          appTargetUrl: "/?shop=${encodeURIComponent(shop)}"
        }

        if (window.top == window.self) {
          // If the current window is the 'parent', change the URL by setting location.href
          window.top.location.href = targetInfo.hasStorageAccessUrl;
        } else {
          var storageAccessHelper = new StorageAccessHelper(targetInfo);
          storageAccessHelper.execute();
        }
      }

      document.addEventListener("DOMContentLoaded", redirect);
    })();`;
};
export default requestStorageAccess;
